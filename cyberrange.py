#!/usr/bin/env python3
"""
VERSION 2: LLM-driven cyber-physical range + safe Docker profiles + multi-output GP + active intervention policy
...
# Version 2 tracks the four modeled CPS assets in every profile. Enhanced and
# laptop Compose files add monitoring/IDS services, but those infrastructure
# containers are not simulator assets and are therefore not passed to CPSRange.
ENHANCED_CONTAINERS = STANDARD_CONTAINERS
LAPTOP_CONTAINERS = STANDARD_CONTAINERS
...
# Dataset/PCAP export: deterministic addresses when Docker is disabled.
# Version 2 models exactly these four CPS assets in every simulator profile;
# monitoring and IDS containers are infrastructure, not attack targets.
...
# Modeled asset -> default ports for synthetic PCAP output.
  ...
  plc_industrial_01:
    ...
    command:
      [
        "sh",
        "-c",
        "apk add --no-cache busybox-extras && mkdir -p /plc && echo 'SAFE_v2' > /plc/logic_hash && sleep infinity"
      ]
    networks:
      ot_net:
    ...
    def _blue_step(self, a: str, target: str, service: str, params: Dict[str, Any]) -> str:
        ...
        if a == "RESTORE":
            if asset.kind == "plc":
                ...
                asset.notes["logic_hash"] = "SAFE_v2"
...
class PrometheusMetrics:
    ...
    def update_round(self, env: "CPSRange", red_action: str, red_result: str, blue_action: str, blue_result: str) -> None:
        ...
        logic_hash = str(env.assets.get(PLC_ASSET_ID).notes.get("logic_hash", "SAFE_v2")) if PLC_ASSET_ID in env.assets else "SAFE_v2"
...
def _apply_passive_compromise_effects(env: "CPSRange") -> None:
    ...
    if plc and plc.compromised and plc.privilege == "ADMIN":
        logic_hash = plc.notes.get("logic_hash", "SAFE_v2")
...
def main() -> None:
    ...
    ap.add_argument("--enhanced-docker", action="store_true",
                    help="Use the safe enhanced Compose profile with monitoring and offline IDS validation.")
    ap.add_argument("--laptop-docker", action="store_true",
                    help="Use laptop-optimized lightweight docker-compose.")
    ap.add_argument(
        "--write-compose", action="store_true",
        help="Explicitly overwrite --compose with the embedded standard compose definition.",
    )
    ap.add_argument("--no-write-compose", action="store_true", help=argparse.SUPPRESS)
    ...
    ap.add_argument("--neural-arch", type=str, default="deep_feedforward",
                    choices=["deep_feedforward", "deep"],
                    help="Integrated decision architecture for neural agents.")
    ...
    ap.add_argument(
        "--allow-external-modbus", action="store_true",
        help="Acknowledge read/write access to a non-loopback Modbus endpoint.",
    )
    ...
    if args.multi_agent and args.neuroevolution:
        ap.error("--neuroevolution is not integrated with the Version 2 simulator loop; use advanced_neural_architectures.py directly")
    if args.real_modbus:
        try:
            modbus_address = ipaddress.ip_address(args.modbus_host)
        except ValueError:
            modbus_address = None
        is_loopback = bool(modbus_address and modbus_address.is_loopback) or args.modbus_host.lower() == "localhost"
        if not is_loopback and not args.allow_external_modbus:
            ap.error(
                "Refusing non-loopback --real-modbus endpoint without "
                "--allow-external-modbus. This mode reads from and writes to the target device."
            )
    ...
    if args.enhanced_docker:
        compose_path = os.path.abspath("monitoring/docker-compose-enhanced.yml")
        CONTAINER_NAMES = ENHANCED_CONTAINERS
        _log_info("[INFO] Using safe enhanced Compose profile with monitoring and offline IDS validation")
        _log_info("[INFO] Modeled CPS assets: %d", len(ENHANCED_CONTAINERS))
    ...
    if args.write_compose and not args.no_write_compose:
        if args.enhanced_docker or args.laptop_docker:
            ap.error("--write-compose only supports the standard --compose path")
    ...
    if dr is not None:
        ...
        for container_name in CONTAINER_NAMES:
            try:
                # Prefer the logical network used by the Version 2 Compose files.
                if container_name in ("gw_dmz_01", "hist_data_01"):
                    network = "dmz_net"
                elif container_name in ("hmi_ops_01", "plc_industrial_01"):
                    network = "ot_net"
                else:
                    network = "sim"
    ...
    last_plc_hash = env.assets[PLC_ASSET_ID].notes.get("logic_hash", "SAFE_v2")
    dataset_rows: List[Dict[str, Any]] = []
    ...
    try:
        ...
        while True:
            ...
            if dr is not None and not args.no_container_markers:
                ...
                plc_hash = env.assets[PLC_ASSET_ID].notes.get("logic_hash", "SAFE_v2")
